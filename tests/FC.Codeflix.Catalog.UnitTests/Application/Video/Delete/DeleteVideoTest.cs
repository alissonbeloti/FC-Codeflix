...
public class DeleteVideoTest
{
    ...
    public DeleteVideoTest(DeleteVideoTestFixture fixture)
    {
        ...
        _useCase = new UseCase.DeleteVideo(_repositoryMock.Object, 
    ...
    [Trait("Application", "DeleteVideo - Use Cases")]
    private async Task DeleteVideoWithAllMediasAndClearStorage()
    {
        var videoExample = _fixture.GetValidVideo();
    ...
    [Trait("Application", "DeleteVideo - Use Cases")]
    private async Task DeleteVideoWithOnlyTrailerAndClearStorageOnlyForTrailer()
    {
        var videoExample = _fixture.GetValidVideo();
        videoExample.UpdateTrailer(_fixture.GetValidMediaPath());
        var input = _fixture.GetValidInput(videoExample.Id);
    ...
    [Trait("Application", "DeleteVideo - Use Cases")]
    private async Task DeleteVideoWithOnlyMediaAndClearStorageOnlyForMedia()
    {
        var videoExample = _fixture.GetValidVideo();
        videoExample.UpdateMedia(_fixture.GetValidMediaPath());
        var input = _fixture.GetValidInput(videoExample.Id);
        ...
        _unitOfWorkMock.Verify(x => x.Commit(It.IsAny<CancellationToken>()), Times.Once);
        _storageService.Verify(x => x.Delete(It.Is<string>(
            filePath => filePath == videoExample.Media!.FilePath),
            It.IsAny<CancellationToken>()), Times.Once);
        _storageService.Verify(x => x.Delete(It.IsAny<string>(),
            It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact(DisplayName = nameof(DeleteVideoWithoutMediaAndDontClearStorage))]
    [Trait("Application", "DeleteVideo - Use Cases")]
    private async Task DeleteVideoWithoutMediaAndDontClearStorage()
    {
        var videoExample = _fixture.GetValidVideo();
        var input = _fixture.GetValidInput(videoExample.Id);
        _repositoryMock.Setup(x => x.Get(
    ...
    [Trait("Application", "DeleteVideo - Use Cases")]
    private async Task ThrowsNotFoundExceptionWhenVideoNotFound()
    {
