...
namespace FC.Codeflix.Catalog.UnitTests.Domain.Extentions;
public class RatingExtensionsTest
{
    [Theory(DisplayName = nameof(StringToRating))]
    [Trait("Domaing", "Rating - Extensions")]
    ...
    public void RatingToString(Rating rating, string expectedEnumString) => 
        rating.ToStringSignal().Should().Be(expectedEnumString);
}
