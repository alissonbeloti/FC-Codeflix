...
public class ListCategoryTest
{
    ...
    public async Task SearchReturnsListAndTotal()
    {
        ...
        var serachInput = new ListCategoriesInput(1, 20);
        var useCase = new ListCategories(categoryRepository);

        var output = await useCase.Handle(serachInput, CancellationToken.None);

        output.Should().NotBeNull();
        output.Items.Should().NotBeNull();
        output.Page.Should().Be(serachInput.Page);
    ...
    [InlineData(7, 2, 5, 2)]
    [InlineData(7, 3, 5, 0)]
    public async Task SearchReturnsPaginated(int quantityToGenerate, int page, int perPage,
        int expectedQuantityItems)
    {
        CodeflixCatalogDbContext dbContext = _fixture.CreateDbContext();
        var exampleCategoryList = _fixture.GetExampleCategoryList(quantityToGenerate);
        await dbContext.AddRangeAsync(exampleCategoryList);
        await dbContext.SaveChangesAsync(CancellationToken.None);
        var categoryRepository = new CategoryRepository(dbContext);
        var serachInput = new ListCategoriesInput(page, perPage);
        var useCase = new ListCategories(categoryRepository);
       ...
       int expectedQuantityTotalItems)
    {
        ...
        var categoryExampleList = _fixture.GetExampleCategoriesListWithName(categoryNamesList);

        await dbContext.AddRangeAsync(categoryExampleList);
        await dbContext.SaveChangesAsync(CancellationToken.None);
        var categoryRepository = new CategoryRepository(dbContext);
        var serachInput = new ListCategoriesInput(page, perPage, search);
        var useCase = new ListCategories(categoryRepository);
        ...
        foreach (CategoryModelOutput outputItem in output.Items)
        {
            var exampleItem = categoryExampleList.Find(ecl => ecl.Id == outputItem.Id);
    ...
    public async Task SearchOrdered(string orderBy, string order)
    {
        ...
        var serachInput = new ListCategoriesInput(1, 20, string.Empty, orderBy, useCaseOrder);
        var useCase = new ListCategories(categoryRepository);

        var output = await useCase.Handle(serachInput, CancellationToken.None);

        var orderedItems = _fixture.CloneCategoriesListOrdered(exampleCategoryList, serachInput.Sort, serachInput.Dir);
        output.Should().NotBeNull();
        output.Items.Should().NotBeNull();
        output.Page.Should().Be(serachInput.Page);
        output.PerPage.Should().Be(serachInput.PerPage);
