...
public class VideosController: ControllerBase
{
    ...
    {
        var output = await _mediator.Send(new GetVideoInput(id), cancellation);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return CreatedAtAction(nameof(CreateVideo), new { id = output.Id }, 
            new ApiResponse<VideoModelOutput>(output));
    }

    [HttpGet]
    [ProducesResponseType(typeof(ListVideoOutput), StatusCodes.Status200OK)]
    ...
    public async Task<IActionResult> UpdateVideo(
        [FromRoute] Guid id,
        [FromBody] UpdateVideoApiInput request,
        CancellationToken cancellationToken)
    {
        var input = request.ToFromUpdateVidoInput(id);
        var output = await _mediator.Send(input, cancellationToken);
        return Ok(new ApiResponse<VideoModelOutput>(output));
    }

    [HttpDelete("{id:guid}")]
    [ProducesResponseType(typeof(ApiResponse<VideoModelOutput>), StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status404NotFound)]
    ...
    public async Task<IActionResult> UploadMedia([FromRoute] Guid id,
        ...
        CancellationToken cancellationToken)
    {
        var input = apiInput.ToUploadMediaInput(id, type);
        await _mediator.Send(input, cancellationToken);
        return NoContent();
    }
}
